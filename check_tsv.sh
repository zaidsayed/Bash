...
correlation_results="correlation_results.tsv"

# Extract necessary columns
awk -F'\t' 'BEGIN {OFS="\t"}
    NR == 1 {
        for (i = 1; i <= NF; i++) {
            if ($i == "Entity/Country") country_idx = i
            if ($i == "GDP per capita") gdp_idx = i
            if ($i == "Population") pop_idx = i
            if ($i == "Homicide Rate") hom_idx = i
            if ($i == "Life Expectancy") life_idx = i
            if ($i == "Cantril Ladder score") cantril_idx = i
        }
    }
    NR > 1 && $cantril_idx != "" && $gdp_idx != "" && $pop_idx != "" && $hom_idx != "" && $life_idx != "" {
        print $country_idx, "GDP per capita", $gdp_idx, $cantril_idx
        print $country_idx, "Population", $pop_idx, $cantril_idx
        print $country_idx, "Homicide Rate", $hom_idx, $cantril_idx
        print $country_idx, "Life Expectancy", $life_idx, $cantril_idx
    }
...
# Cleanup
rm "$temp_data" "$correlation_results"
